...
/**
 * The instances processor reads the files located in the instances directory by means of the IO object, merges them and
 * creates a single model with all of them.
 */
trait InstancesProcessor {
   ...
   */
  def getInstancesModel: Either[Exception, RDFAsRDF4jModel]
}
